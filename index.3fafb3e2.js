...
var _gsapMotionPathPlugin = require('gsap/MotionPathPlugin');
_gsap.gsap.registerPlugin(_gsapMotionPathPlugin.MotionPathPlugin);
let tl;
const setAnimation = () => {
  ...
  const crp = [['#c11', '#cr11'], ['#c12', '#cr12'], ['#c13', '#cr13']];
  crp.forEach((c, i) => tl.to(c[0], {
    duration: 5,
    stagger: .25 * i,
    motionPath: {
      path: c[1],
...
const unsetScrollAnimation = () => {
  _gsap.gsap.to('#scroll-svg', {
    opacity: 0,
    y: 50,
    ease: 'Power4.easeOut'
  });
...
(function () {
  ...
  const opens = document.querySelector('#open-scroll');
  const closes = document.querySelector('#close-scroll');
  // gsap.set('#Плашка', { opacity: 1, scale: 1 });
  open.addEventListener('click', setAnimation);
  close.addEventListener('click', unsetAnimation);
  opens.addEventListener('click', setScrollAnimation);
  closes.addEventListener('click', unsetScrollAnimation);
})();

},{"gsap":"1iecp","gsap/MotionPathPlugin":"1pad0"}],"1iecp":[function(require,module,exports) {
var define;
...
},{}],"1pad0":[function(require,module,exports) {
var _parcelHelpers = require("@parcel/transformer-js/lib/esmodule-helpers.js");
_parcelHelpers.defineInteropFlag(exports);
_parcelHelpers.export(exports, "MotionPathPlugin", function () {
...
var _utilsMatrixJs = require("./utils/matrix.js");
var _xProps = ("x,translateX,left,marginLeft,xPercent").split(","), _yProps = ("y,translateY,top,marginTop,yPercent").split(","), _DEG2RAD = Math.PI / 180, gsap, PropTween, _getUnit, _toArray, _getGSAP = function _getGSAP() {
  ...
  if (vars.type === "cubic") {
    ...
    var pointFunc = y ? _utilsPathsJs.pointsToSegment : _utilsPathsJs.flatPointsToSegment;
    segment = [pointFunc(segment, vars.curviness)];
  }
  segment = slicer(_align(segment, target, vars));
  _addDimensionalPropTween(plugin, target, x, segment, "x", unitX);
  y && _addDimensionalPropTween(plugin, target, y, segment, "y", unitY);
  return _utilsPathsJs.cacheRawPathMeasurements(segment, vars.resolution || (vars.curviness === 0 ? 20 : 12));
  ...
  var m = _utilsMatrixJs.getGlobalMatrix(element), svg, x, y;
  ...
  var parentMatrix = _utilsMatrixJs.getGlobalMatrix(fromElement.parentNode, true, true), m = parentMatrix.clone().multiply(_utilsMatrixJs.getGlobalMatrix(toElement)), fromPoint = _originToPoint(fromElement, fromOrigin, parentMatrix), _originToPoint2 = _originToPoint(toElement, toOrigin, parentMatrix), x = _originToPoint2.x, y = _originToPoint2.y, p;
  m.e = m.f = 0;
  ...
  if (!rawPath || !rawPath.length) {
    return _utilsPathsJs.getRawPath("M0,0L0,0");
  }
  if (align) {
    if (align === "self" || (alignTarget = _toArray(align)[0] || target) === target) {
      _utilsPathsJs.transformRawPath(rawPath, 1, 0, 0, 1, curX - x, curY - y);
    } else {
      ...
      _utilsPathsJs.transformRawPath(rawPath, m.a, m.b, m.c, m.d, curX + m.e - (p.x - m.e), curY + m.f - (p.y - m.f));
    }
  }
  if (matrix) {
    _utilsPathsJs.transformRawPath(rawPath, matrix.a, matrix.b, matrix.c, matrix.d, matrix.e, matrix.f);
  } else if (offsetX || offsetY) {
    _utilsPathsJs.transformRawPath(rawPath, 1, 0, 0, 1, offsetX || 0, offsetY || 0);
  }
  return rawPath;
  ...
  return function (rawPath) {
    return start || end !== 1 ? _utilsPathsJs.sliceRawPath(rawPath, start, end) : rawPath;
  };
};
var MotionPathPlugin = {
  ...
  init: function init(target, vars) {
    ...
    if (Array.isArray(path) && !(("closed" in path)) && typeof firstObj !== "number") {
      ...
      rawPath = slicer(_align(_utilsPathsJs.getRawPath(vars.path), target, vars));
      _utilsPathsJs.cacheRawPathMeasurements(rawPath, vars.resolution);
  ...
  render: function render(ratio, data) {
    ...
    while (i--) {
      _utilsPathsJs.getPositionOnPath(rawPaths[i], ratio, !i && data.rotate, rawPaths[i]);
    }
    while (pt) {
  ...
  getLength: function getLength(path) {
    return _utilsPathsJs.cacheRawPathMeasurements(_utilsPathsJs.getRawPath(path)).totalLength;
  },
  sliceRawPath: _utilsPathsJs.sliceRawPath,
  getRawPath: _utilsPathsJs.getRawPath,
  pointsToSegment: _utilsPathsJs.pointsToSegment,
  stringToRawPath: _utilsPathsJs.stringToRawPath,
  rawPathToString: _utilsPathsJs.rawPathToString,
  transformRawPath: _utilsPathsJs.transformRawPath,
  getGlobalMatrix: _utilsMatrixJs.getGlobalMatrix,
  getPositionOnPath: _utilsPathsJs.getPositionOnPath,
  cacheRawPathMeasurements: _utilsPathsJs.cacheRawPathMeasurements,
  convertToPath: function convertToPath(targets, swap) {
    return _toArray(targets).map(function (target) {
      return _utilsPathsJs.convertToPath(target, swap !== false);
    });
  },
  convertCoordinates: function convertCoordinates(fromElement, toElement, point) {
    var m = _utilsMatrixJs.getGlobalMatrix(toElement, true, true).multiply(_utilsMatrixJs.getGlobalMatrix(fromElement));
  ...
  arrayToRawPath: function arrayToRawPath(value, vars) {
    ...
    vars.relative && _relativize(segment);
    return [vars.type === "cubic" ? segment : _utilsPathsJs.pointsToSegment(segment, vars.curviness)];
  }
};
_getGSAP() && gsap.registerPlugin(MotionPathPlugin);

},{"./utils/paths.js":"WR6s2","./utils/matrix.js":"6MuMP","@parcel/transformer-js/lib/esmodule-helpers.js":"21LXs"}],"WR6s2":[function(require,module,exports) {
var _parcelHelpers = require("@parcel/transformer-js/lib/esmodule-helpers.js");
_parcelHelpers.defineInteropFlag(exports);
...
},{}],"6MuMP":[function(require,module,exports) {
var _parcelHelpers = require("@parcel/transformer-js/lib/esmodule-helpers.js");
_parcelHelpers.defineInteropFlag(exports);
_parcelHelpers.export(exports, "Matrix2D", function () {
  return Matrix2D;
});
_parcelHelpers.export(exports, "getGlobalMatrix", function () {
  return getGlobalMatrix;
});
_parcelHelpers.export(exports, "_getDocScrollTop", function () {
  return _getDocScrollTop;
});
_parcelHelpers.export(exports, "_getDocScrollLeft", function () {
  return _getDocScrollLeft;
});
_parcelHelpers.export(exports, "_setDoc", function () {
  return _setDoc;
});
/*!
* matrix 3.6.1
...
var _doc, _win, _docElement, _body, _divContainer, _svgContainer, _identityMatrix, _transformProp = "transform", _transformOriginProp = _transformProp + "Origin", _hasOffsetBug, _setDoc = function _setDoc(element) {
  ...
  while (e && e !== _body) {
    ...
    // force re-parsing of transforms if necessary
...
_divTemps = [], _getDocScrollTop = function _getDocScrollTop() {
  ...
  if (svg) {
    ...
    if (m.numberOfItems) {
      ...
      // don't call m.consolidate().matrix because a bug in Firefox makes pointer events not work when consolidate() is called on the same tick as getBoundingClientRect()! See https://greensock.com/forums/topic/23248-touch-is-not-working-on-draggable-in-firefox-windows-v324/?tab=comments#comment-109800
      x = m.a * b.x + m.c * b.y;
      y = m.b * b.x + m.d * b.y;
    } else {
      m = _identityMatrix;
...
},{"@parcel/transformer-js/lib/esmodule-helpers.js":"21LXs"}]},["3Imd1","5rkFb"], "5rkFb", "parcelRequirea021")
